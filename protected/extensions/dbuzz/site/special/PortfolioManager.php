...
class PortfolioManager{
    ...
    * 
    */
    public function getAllContent($tipo = "produto", $start = 0, $args = array()){
        ...
        $select  = "id, id_categoria, descricao, date_start, date_end, nome, preco_real, cidade, pais, parcelas, ";
        $select .= "unidades_current, marca, data, last_update, tipo, unidades_min, id_master, url, referencia, descricao_resumo";
      
        $sql = "SELECT $select FROM ecommerce_produtos WHERE tipo = 'portfolio' AND exibe_produtos = 1 ORDER BY id_categoria DESC, n_index ASC LIMIT $start, $limite";
        ...
        try{          
            ...
            echo 'ERROR: PortifolioManager - getAllContent() '. $e->getMessage();
        }   
        return $recordset;
    }
    ...
    * 
    */
    public function getAllContentPrincipal($tipo = "produto"){
        ...
        $sql = "SELECT $select FROM ecommerce_categorias WHERE tipo = 1 AND exibe = 1 ORDER BY n_index ASC";
        ...
        try{          
            ...
            Yii::trace("ERROR ".$e->getMessage());
            echo 'ERROR: PortifolioManager - getContentPrincipal() '. $e->getMessage();
        }   
    ...
    */
    public function getContentById($id, $view = "", $id_estoque = false){
        ...
        $select .= "preco_real, date_start, date_end, marca, entrega, cidade, pais, id_pedido, id_subcategoria, id_subitem, tipo, referencia, ";
        ...
        try{
            ...
            Yii::trace("ERROR ".$e->getMessage());
            echo 'ERROR: PortifolioManager - getContentById() '.$e->getMessage();
        }
    }
     ...
     *
    */
    public function getAllContentByCategoria($cat, $sub, $subitem, $start, $order){
        ...
        if($cat != "" && $sub == "" && $id_categoria){
            $sql = "SELECT id, id_categoria FROM ecommerce_produtos WHERE exibe_produtos = 1 AND id_categoria = {$id_categoria['id_categoria']} {$order['order_by']} LIMIT $start, {$order['max']}";
        }       
        
        if($sub != "" && $subitem == "" && $id_categoria){
            $sql = "SELECT id, id_categoria, id_subcategoria FROM ecommerce_produtos WHERE exibe_produtos = 1 AND id_subcategoria = {$id_categoria['id_subcategoria']}"; 
        }
        
        if($sub != "" && $subitem != "" && $id_categoria){
            $sql = "SELECT id, id_categoria, id_subcategoria, id_subitem, referencia FROM ecommerce_produtos WHERE exibe_produtos = 1 AND id_subitem = {$id_categoria['id_subitem']}"; 
        }
        ...
        try{
            ...
            Yii::trace("ERROR ".$e->getMessage());
            echo 'ERROR: PortifolioManager - getAllContentByCategoria() '. $e->getMessage();
        }
    }
    ...
    */
    public function getContentByIdType($id, $type){
        ...
        switch($type){
            ...
            //Existe um metodo igual em PortifolioManager - Admin... é diferente um pouco
            case "ordem_servico":
                $sql = "SELECT $select FROM ecommerce_produtos WHERE ordem_servico = 1 ORDER BY nome ASC";
                break;
        }
        ...
        try{
            ...
            Yii::trace("ERROR ".$e->getMessage());
            echo 'ERROR: PortifolioManager - getContentByType() '. $e->getMessage();
        }
    }    
     ...
     *
    */
    public function getMenuPortfolio(){
 
        Yii::import('application.extensions.utils.StoreUtils');
        
        $select  = "id_categoria, categoria_label, categoria_url";        
        $sql = "SELECT $select FROM ecommerce_categorias WHERE tipo = 1 AND exibe = 1";
        ...
        try{
            ...
            Yii::trace("ERROR ".$e->getMessage());
            echo 'ERROR: PortifolioManager - getMenuEcommerce() '. $e->getMessage();
        }
    }
    ...
    */
    public function getContentByIdAttribute($attribute){
        ...
        $sql = "SELECT ".$select." FROM ecommerce_produtos WHERE $attribute = 1 ";
        ...
        try{
            ...
            Yii::trace("ERROR ".$e->getMessage());
            echo 'ERROR: PortifolioManager - getContentByIdAttribute() '. $e->getMessage();
        }
    }
