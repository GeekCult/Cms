...
function pageselectCallback(page_index, jq){
    var new_content = jQuery('#images_support div.imageFotos:eq('+page_index+')').clone();
	...
	$.fn.pagination = function(maxentries, opts){
		...
		function selectPage(new_current_page) {
			...
			var continuePropagation = opts.callback(new_current_page, containers);
                        var isPlace = $("#helper_place_type").val();
                        var idpage = $("#helper").val();
                        var type_image  = $("#helper_type_images").val();
                        
                        //Verify if is admin or if it is not set yet. 
                        if(isPlace == "" || isPlace == undefined){isPlace = "admin";} 
                        $("#Searchresult").load("/"+isPlace+"/" +  local_page +  "/paginar_fancy_"+type_image+"/" + new_current_page + "0/" + idpage);                      
