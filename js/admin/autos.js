/*
    Document   : autos admin
    Created on : 31/09/2015, 16:31:00
    Author     : CarlosGarcia
    Description:
...
$(document).ready(function(){
    ...
    if($('#action_helper').val() == 'novo' || $('#action_helper').val() == 'editar') setTimeout("initListenersAutos()", 200);
...
function initListar(){
    ...
    **/
    ...
    $("#categoria").change(function() {
        window.location = "/admin/autos/listar/categorias/" + $("#categoria").val();
    }); 
    ...
    $("#mais_filtros").change(function() {
        $.post("/site/relatar/set_session_data",{id_user: 0,label: 'produtos_mais_filtro', value: $(this).val()},function(data){
            window.location = "/admin/autos/listar/";        
        });
...
function editForm(id){
    window.location = "/admin/autos/editar/id/" + id;
}
...
function submitDetailsProdutos(){
    ...
    $.post("/admin/autos/salvar_detalhes",{
...
function initListenersAutos(){
    ...
    $("#marca").change(function(){
        $.post("/admin/autos/load_modelos", {id: $(this).val()    
    },function(data){
        var jsonObject = eval('(' + data + ')');
    ...
    $('#form-botao-atualizar').click(function(){
       window.location = '/admin/autos/editar/' + $(this).attr('rel'); 
    });
...
function reloadTypeProduct(type){
    ...
    if(type == "programa"){
        $.post("/admin/autos/obter_produtos", {
            id: type
        },function(data){
...
function submitFormProducts(){
    ...
    $.post("/admin/autos/salvar", content,function(data){
        var jsonObject = eval('(' + data + ')');
        showAlertDim(jsonObject['message']);
        setTimeout("tb_removeAlert('common')", 1500);
        $('#action').val('editar');
...
function removeImageFromProduct(id_image, id_produto, name_image){
    ...
    if(id_produto != ''){
        $.post("/admin/autos/remover_image", {
            id_image: id_image,
            id_produto: id_produto,
...
function submitSubItems(){
    ...
    $.post("/admin/autos/cadastrar_categoria",{
...
function removeCategoryEcommerce(id, type){
    ...
    $.post("/admin/autos/remover_categoria",{
        id: id,
        type: type
    },function(data){
        showAlertDim(data);
...
function editCategoryEcommerce(id){
    window.location = "/admin/autos/categorias/editar/" + id;
}
...
function editCategoryEcommerceMain(id){
    window.location = "/admin/autos/categoria_settings/editar/" + id;
}
...
function submitCategoriaSettingsEcommerce(){
    ...
    $.post("/admin/autos/categoria_settings_salvar",{
        nome: nome,
        index: index,
        descricao: descricao,
        id_categoria: id_categoria,
...
function initLojaCategoriasListeners(){
    ...
    if($("#helper_admin_versao").val() == 1){
        ...
        $("#fancybox_subloja_launcher").fancybox({
            ...
            'height'                : 420,
            'overlayShow'           : false,
            'href'                  : "/admin/autos/sub_adicionar",
            'titleShow'             : false
        });
        ...
        $("#add_category_store").click(function(){        
            PurplePier.showModalSmall({url: "/admin/autos/adicionar", height: '70%'});
        });

        $("#add_subcategory_store").click(function(){        
            PurplePier.showModalSmall({url: "/admin/autos/sub_adicionar", height: '70%'});
        });
    }
